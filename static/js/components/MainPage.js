...
export class MainPage extends Component {
  constructor(props) {
    ...
    let url =
      ...
      this.props.match.params.id.toUpperCase().split("-");
    this.state = {
      isLightTheme: parseInt(cookie.load("isLightTheme")),
      data: null,
      asset:
        url && url.length === 2 && Object.keys(assets).includes(url[0])
          ? url[0]
          : "BTC",
      currency:
        url && url.length === 2 && Object.keys(currencies).includes(url[1])
          ? url[1]
          : "INR",
      firstPrice: 0,
  ...
  getCountDown = () => {
    setInterval(() => {
      this.setState((oldState) => {
        return {
          countDownTimer:
            oldState.countDownTimer > 0 ? oldState.countDownTimer - 1 : 60,
        };
      });
    }, 1000);
  };
  ...
  _getData = (clicked) => {
    ...
    this.state.socket.on("FromAPI", (data) => {
      let lastPriceArray = Object.keys(data)
        .map((item) => {
      ...
      let averageLastPrice =
        lastPriceArray.length > 0
          ? (lastPriceSum / lastPriceArray.length).toFixed(0)
          : 0;
      data &&
        this.setState(
          {
            ...
            data: data,
            firstPrice:
              lastPriceArray.length > 0 ? Math.min(...lastPriceArray) : 0,
          ...
          () => {
            document.title =
              ...
              this._getSymbol() +
              this._formatPrice(
                Math.abs(parseInt(this.state.data.best_trade_price).toFixed(0))
              );
          }
        );
    });
  ...
  _getDifference = (netPrice, index) => {
    ...
    if (differencePercent > 0) {
      return (
        <span className={this.state.buyMode ? "color-red" : "color-green"}>
          {" "}
          {this.state.buyMode ? "-" : ""} {differencePercent + " %"}{" "}
        </span>
      );
    } else if (differencePercent < 0) {
      return (
        <span className={this.state.buyMode ? "color-green" : "color-red"}>
          {" "}
          {!this.state.buyMode ? "-" : ""}{" "}
    ...
    if (difference > 0) {
      return (
        <div
          className={
            this.state.buyMode
              ? "color-red d-flex align-items-center "
        ...
        >
          ...
          <div>
            {" "}
            {this._getSymbol()} {this._formatPrice(difference)}{" "}
          </div>{" "}
        </div>
      );
      ...
      return (
        <div className={"color-green d-flex align-items-center "}>
          {" "}
          {this._getSymbol()}{" "}
          {this._formatPrice(parseInt(Math.abs(difference)))}{" "}
        </div>
    ...
    return (
      <tbody>
        ...
        {this.state.data.maindata.map((index, key) => {
          ...
          // if(rank < 11){
          if (
            this.state.asset == "USDT" ||
            this.state.asset == "XRP" ||
            this.state.asset == "TRX" ||
            this.state.asset == "IOST" ||
            this.state.asset == "WIN" ||
            this.state.asset == "BTT" ||
            this.state.asset == "WRX"
          ) {
            // alert("helo")
            return (
              <tr key={key}>
                <td className="align-middle">
                  <h4 className="table-text"> {++rank} </h4>{" "}
                </td>{" "}
                <td className="align-middle">
                  <a target="_blank" href={Exchangelink[rank - 1]}>
                    <h4 className={"table-text"}>
                      {" "}
                      {indianLogos[rank - 1] && (
                        <img
                          src={indianLogos[rank - 1]}
                ...
                <td className="align-middle">
                  ...
                  <h4 className="table-text">
                    {" "}
                    {this._getSymbol()}{" "}
                    {this._formatPriceusdt(
                      Math.abs(
                        parseFloat(
                          this.state.data.maindata[key].last_trade_price
                        ).toFixed(2)
                      )
                    )}{" "}
                  </h4>
                ...
                <td className="align-middle">
                  <h4 className="table-text">
                    <span>
                      ...
                      {this._formatPriceusdt(
                        Math.abs(
                          parseFloat(this.state.data.maindata[key].buy).toFixed(
                            2
                          )
                        )
                      )}
                      ...
                      {this._formatPriceusdt(
                        Math.abs(
                          parseFloat(
                            this.state.data.maindata[key].sell
                          ).toFixed(2)
                        )
                      )}
                    </span>{" "}
                    {/* <span className="commission-percent">{`(${(this.state.data[key].commissionPercent).toFixed(0)} %)`}</span> */}{" "}
                  </h4>{" "}
                </td>{" "}
                ...
                <td className="align-middle">
                  ...
                  {this.state.data.maindata[key].difference < 0 ? (
                    <h4 className="table-text color-red">
                      ...
                      ▼{this._getSymbol()}{" "}
                      {this._formatPriceusdt(
                        Math.abs(this.state.data.maindata[key].saving)
                      )}{" "}
                    </h4>
                  ) : (
                    <h4 className="table-text color-green">
                ...
                <td className="align-middle">
                  <h4 className="table-text">
                    <span>
                      ...
                      {this._formatPrice(
                        Math.abs(
                          parseInt(this.state.data.maindata[key].buy).toFixed(0)
                        )
                      )}
                      / {this._getSymbol()}
                      {this._formatPrice(
                        Math.abs(
                          parseInt(this.state.data.maindata[key].sell).toFixed(
                            0
                          )
                        )
                      )}
                  ...
                  {this.state.data.maindata[key].difference < 0 ? (
                    <h4 className="table-text color-red">
                      ...
                      {this._formatPrice(
                        Math.abs(
                          parseInt(
                            this.state.data.maindata[key].saving
                          ).toFixed(0)
                        )
                      )}{" "}
                    </h4>
  ...
  getRandomObj = () => {
    ...
    let random1hour = parseInt(
      (random5mins + parseInt(Math.random().toFixed(0))).toFixed(0)
    );
    let random1day = parseInt(
      (random1hour + parseInt((Math.random() * 9.9).toFixed(0))).toFixed(0)
    );
    let random7days = parseInt(
      (random1hour + parseInt((Math.random() * 9.9).toFixed(0))).toFixed(0)
    );
    return {
            ...
            >
              <div className="text-center font-32 average-block">
                ...
                >
                  ...
                  {this._formatPriceusdt(
                    Math.abs(
                      parseFloat(this.state.data.best_trade_price).toFixed(2)
                    )
                  )}{" "}
                </div>{" "}
                <div className="average-subText">
                  Average {this.state.asset}/{this.state.currency} net price
            ...
            >
              <div className="text-center font-32 average-block">
                ...
                >
                  ...
                  {this._formatPrice(
                    Math.abs(
                      parseInt(this.state.data.best_trade_price).toFixed(0)
                    )
                  )}{" "}
                </div>{" "}
                <div className="average-subText">
  ...
  render() {
    return (
      <div className={"theme-" + (this.state.isLightTheme ? "light" : "dark")}>
        <Header
          isLightTheme={this.state.isLightTheme}
          onThemeButtonClick={this.onThemeButtonClick}
          asset={this.state.asset}
          currency={this.state.currency}
                  ...
                  <tr>
                    ...
                    <th>
                      <h4>
                        <span
                          className="pointer"
                          onClick={this.setSort.bind(null, "platform")}
                        >
                          Platform{" "}
                        </span>{" "}
                      </h4>{" "}
                    ...
                    <th>
                      <h4>
                        <span
                          className="pointer"
                          onClick={this.setSort.bind(null, "price")}
                        >
                          Last Traded Price{" "}
                        </span>{" "}
                      </h4>{" "}
                    ...
                    <th>
                      <h4>
                        <span
                          className="pointer"
                          onClick={this.setSort.bind(null, "commission")}
                        >
                          Buy / Sell Price{" "}
                        </span>{" "}
                      </h4>{" "}
                    ...
                    <th>
                      <h4>
                        <span
                          className="pointer"
                          onClick={this.setSort.bind(null, "difference")}
                        >
                          Difference{" "}
                        </span>{" "}
                      </h4>{" "}
